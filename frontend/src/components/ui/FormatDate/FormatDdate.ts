...
class FormatDate {
  ...
  constructor(public dataTime: string | Date | number) {
    this.date = dayjs(new Date(dataTime)).format(
      'DD.MM.YYYY HH:mm',
    );
  }
  ...
  getFormatDate() {
    ...
    if (date.current.year === date.date.year) {
      if (
        date.current.day === date.date.day &&
        date.current.month === date.date.month
      ) {
        return date.date.toDayFormat;
      } else if (
        date.current.day - 1 === date.date.day &&
        date.current.month === date.date.month
      ) {
